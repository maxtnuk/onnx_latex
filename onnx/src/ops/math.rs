...
pub fn register_all_ops(reg: &mut OnnxOpRegister) {
    ...
    reg.insert("Sum", |_, _| {
        Ok((Box::new(Nary(Box::new(ops::math::Add), false)), vec![]))
    });
    reg.insert("Max", |_, _| {
        Ok((Box::new(Nary(Box::new(ops::math::Max), false)), vec![]))
    });
    reg.insert("Min", |_, _| {
        Ok((Box::new(Nary(Box::new(ops::math::Min), false)), vec![]))
    });
    reg.insert("Mean", |_, _| {
        Ok((Box::new(Nary(Box::new(ops::math::Add), true)), vec![]))
    });
    ...
    reg.insert("Round", |_, _| {
        Ok((Box::new(ops::math::round_half_to_even()), vec![]))
    });
    ...
    reg.insert("Erf", |_, _| {
        Ok((Box::new(tract_onnx_opl::erf::erf()), vec![]))
    });
    ...
    reg.insert("IsNaN", |_, _| {
        Ok((Box::new(tract_onnx_opl::is_nan::is_nan()), vec![]))
    });
    reg.insert("IsInf", isinf);
    reg.insert("Neg", |_, _| Ok((Box::new(ops::math::neg()), vec![])));
    reg.insert("Sign", |_, _| Ok((Box::new(ops::math::sign()), vec![])));
    reg.insert("Reciprocal", |_, _| {
        Ok((Box::new(ops::math::recip()), vec![]))
    });

    reg.insert("Pow", pow::pow);

    reg.insert("MatMul", |_, _| {
        Ok((expand(ops::matmul::MatMulInference::default()), vec![]))
    });
...
fn isinf(
    ...
    Ok((
        Box::new(tract_onnx_opl::is_inf::is_inf(
            detect_positive,
            detect_negative,
        )),
        vec![],
