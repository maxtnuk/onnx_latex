...
impl Expansion for Conv {
    ...
    fn rules<'r, 'p: 'r, 's: 'r>(
        ...
        if inputs.len() < 2 {
            bail!(
                "Wrong number of inputs. Expected 2 or more, got {}",
                inputs.len()
            );
        }
        ...
        if let Some(kshape) = &self.kernel_shape {
            ...
            for (ix, dim) in kshape.iter().enumerate() {
                s.equals(
                    &k_input.shape[ix + self.kernel_fmt.h_axis()],
        ...
        s.equals(
            &inputs[0].rank,
            k_input.rank.bex() + (has_n as usize as i64 - 1),
        )?;
        ...
        s.given_2(&inputs[0].rank, &k_input.rank, move |s, irank, krank| {
            ...
            s.equals(
                input_c.bex(),
                self.group.unwrap_or(1) as i64 * filter_i.bex(),
            )
        })?;
        s.given_2(
            &inputs[0].shape,
    ...
    fn wire(
        ...
        let input_shape = self
            .data_format
            .shape(input.shape.iter().collect::<TVec<_>>())?;
        ...
        let bias = if let Some(slot) = self.bias_input {
            Some(
                model
                    .outlet_fact(inputs[slot])?
                    .konst
                    .clone()
                    .context("Bias must be const")?,
            )
        } else {
            None
        };
        ...
        let q_params = if quantized {
            ...
            let a_scale = if let Some(o) = self.k_scale_input {
                o.into()
            } else {
                one.clone()
            };
            ...
            let b_scale = if let Some(o) = self.x_scale_input {
                o.into()
            } else {
                one.clone()
            };
            ...
            let c_scale = if let Some(o) = self.y_scale_input {
                o.into()
            } else {
                one.clone()
            };
...
mod test {
    ...
    fn test_infer_with_known_kshape() {
        ...
        let facts = op
            .infer_facts(tvec!(&ifact, &kfact), tvec!(&ofact), tvec!())
            .unwrap();
        assert_eq!(
            facts.1,
            tvec!(InferenceFact::dt_shape(
                DatumType::F32,
                shapefactoid!(1, 1, 3, 2)
            ))
        );
    }
    ...
    fn test_eval_nhwc_1() -> TractResult<()> {
        ...
        let res = op.eval(tvec!(
            Tensor::zero::<f32>(&[1, 2, 2, 2])
                .unwrap()
                .into_arc_tensor(),
            Tensor::zero::<f32>(&[2, 2, 2, 1])
                .unwrap()
                .into_arc_tensor(),
        ))?;
        Tensor::zero::<f32>(&[1, 2, 2, 1])
            .unwrap()
            .close_enough(&res[0], false)
    }
